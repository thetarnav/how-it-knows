...
export const local_storage = {
    ...
    ls_set: function (k_ptr: number, k_len: number, v_ptr: number, v_len: number): void {
        const key = load_string(wasm_memory.buffer, k_ptr, k_len)
        const val = load_string(wasm_memory.buffer, v_ptr, v_len)
